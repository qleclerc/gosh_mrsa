...
#   select(project_id, date)
# 
# all_theatre = rbind(theatre1, theatre2)
# 
# all_theatre %>%
#   mutate(date = floor_date(date, "year")) %>%
#   group_by(date) %>%
...
# theatre1 = read.csv(here::here("Data", "caboodle_patient_theatre_list.csv")) %>%
#   mutate(date = as_date(start_datetime)) %>%
#   select(project_id, date, spect_name)
# theatre2 = read.csv(here::here("Data", "datalake_patient_theatre_list.csv")) %>%
#   mutate(date = as_date(start_datetime)) %>%
#   select(project_id, date, spect_name)
# 
...
# all_theatre %>%
#   mutate(date = floor_date(date, "month")) %>%
#   group_by(date, spect_name) %>%
#   summarise(n = n()) %>%
#   ggplot() +
#   geom_line(aes(date, n, colour = spect_name)) +
#   theme_bw()
# 
# 
# mrsa_n = staph_isolates %>%
#   filter(SpeciesName == "Methicillin-Resistant Staphylococcus aureus") %>%
#   mutate(date = floor_date(date, "month")) %>%
#   group_by(date) %>%
#   summarise(n = n())
# 
# for(i in unique(all_theatre$spect_name)){
#   
#   tt = all_theatre %>%
#     filter(spect_name == i) %>%
#     mutate(date = floor_date(date, "month")) %>%
#     group_by(date) %>%
#     summarise(n_ops = n()) %>%
#     right_join(mrsa_n, by = "date")
#   
#   if(sum(!is.na(tt$n_ops)) < 4) next
#   
#   ctt = cor.test(tt$n_ops, tt$n)
#   
#   if(is.na(ctt$p.value)) next
#   
#   if(ctt$p.value < 0.05){
#     cat(i, "cor", ctt$estimate, "p val", ctt$p.value, "\n")
#   }
#   
# }
# 
# all_theatre %>%
#   filter(spect_name %in% c("Paediatric Cardiology", "Paediatric Surgery", "Endocrinology",
...
# icu_patients %>%
#   mutate(date = floor_date(date, "month")) %>%
#   group_by(date, ward_code) %>%
#   summarise(n = n()) %>%
#   filter(ward_code %in% c("DU", "EAGLD", "EAGLE HAEMOD", "PICU", "NICU", "CATS",
#                           "CICU", "FLAMI", "ALLIGATOR")) %>%
#   ggplot()+
#   geom_line(aes(date, n, colour = ward_code)) +
#   theme_bw()
# 
# 
# #descriptive stats
